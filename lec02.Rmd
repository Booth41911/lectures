...
3. $|\beta_*| = s$
\[
\frac{\Expect{\norm{\X\beta_*-\X\hat\beta}}/n}{s\sigma^2/n} \leq 4\log p + 2 + o(1).
\]

They, also prove a lower bound 
\[
\inf_{\hat\beta}\sup_{\X,\beta_*} \frac{\Expect{\norm{\X\beta_*-\X\hat\beta}}/n}{s\sigma^2/n} \geq 2\log p - o(\log p).
\]

~~Important:~~ 
- even if we could compute the subset selection estimator at scale, it’s clear that we would want to
