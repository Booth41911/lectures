...
\Rightarrow & p(y \given \set{X=1}) - p(y \given \set{X=1}) = \int p(z)p(y\given 1,z)dz - \int p(z)p(y\given 0,z)dz
\end{aligned}
\]
...
* Rather than computing plug-in regression estimators for $\mu(x=1,\cdot)$ and $\mu(x=1,\cdot)$, we could try __matching__
...
- \alr{require extensive random access to matrix}
